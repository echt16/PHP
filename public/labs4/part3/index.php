...
use part3\Product;
use part3\Phone;
use part3\MonitorCategory;
use part3\PhoneCategory;
use part3\Session;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['productToCart'])) {
    $values = explode(';', $_POST['productToCart']);
    $category = $values[0];
    $productToCart = $values[1];
    foreach ($categories[$category]->listProduct as $product) {
        if ($product->name == $productToCart) {
            $session->productList[] = $product;
...
<?php
foreach ($_SESSION['session']->getCart() as $product) {
        ...
        <ul class="product-list">
            <?php if (!empty($selectedItems)): ?>
                <?php foreach ($selectedItems as $item): ?>
                    <form method="post">
                        <input type="hidden" name="productToCart" value="<?= $category . ';' . $item->name; ?>">
